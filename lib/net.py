...
class Net():
    def __init__(self, pt, model=None, phase = caffe.TEST, noTF=1, accname=None, gt_pt=None, gt_model=None):
        ...
        if gt_pt is not None:
            print("using ground-truth model: ", gt_pt ,gt_model)
        ...
        self.norms = self.type2names(layer_type='LRN')

        # TODO: make this flags be initilized with their currect value by reading the prototxt
        self.noBNs = None
        self.noLRNs = None
    ...
    @property
    def _frozen_layer(self):
        if cfgs.frozenname is None:
            frozenname = 'frozen_layer'
        else:
            frozenname = cfgs.frozenname
        return osp.join(osp.split(self.pt_dir)[0], frozenname)
    ...
    def param_data(self, name): # wrapper, gets a layer data
        return self.param_w(name).data

    def param_b_data(self, name): # wrapper, gets a layer bias
    ...
    def set_bn(self, bn, new_name=None):
        if new_name is not None:
            self.net_param.ch_name(bn, new_name)

    def set_fc(self, fc, new_name=None):
        if new_name is not None:
            self.net_param.ch_name(fc, new_name)
    ...
    def extract_features(self, names=[], nBatches=None, points_dict=None, save=False):
        ...
        print("acc coming form extract_features")
        self.clr_acc(show=True)
        if save:
            if frozen_points:
    ...
    def extract_XY(self, X, Y, DEBUG = False, w1=None):
        ...
        for batch in range(nBatches):
            ...
            idx += nFeatsPerBatch
        print("acc coming form extract_XY()")
        self.clr_acc(show=True)
        return feats_dict
    ...
    #insert inserts layers into the prototxt
    ...
    def seperateConvReLU(self, DEBUG=0):
        if DEBUG_Mario: print("seperateConvReLU() separating ReLU layers from Conv layers." )
    ...
    def R3(self):
        ...
        def setConv(c, d): # sets the weiths of a layer (column of the kernel matrix -- filters -- ) to specific set of values
    ...
    def R1(self):
        ...
        prefix = '1Cx'
        DEBUG = True
        alldic = dcfgs.model.alldic
        pooldic = dcfgs.model.pooldic
        d_cdic = dcfgs.model.d_cdic
        convs= self.convs
        self.WPQ = dict()
        self.selection = dict()
        self._mem = True
        ...
        for conv, convnext in zip(convs[0:], convs[1:]):
            ...
            if (conv in alldic or conv in pooldic) : #and (convnext in self.convs):
                ...
                if DEBUG_Mario:
                    print("-"*30); print("\t channel pruning "); print("-"*30)
                    print("%s , operating: conv = %s , convnext = %s, d_c = %s" % (dcfgs.model.model,conv,convnext,d_c))

                #if conv.startswith('conv4'): #what is this -by Mario
                #    c_ratio = 1.5
                if conv in self.selection:
                    if DEBUG_Mario:
                        print(" @  Prune filter chanels (W2 from last iter) @")
                        print(" reshape WPQ[(conv,0)] : ",self.param_data(conv).shape )
                    self.WPQ[(conv_P,0)] =  self.param_data(conv)[:,self.selection[conv],:,:]
                    self.WPQ[(conv_P,1)] =  self.param_b_data(conv)
                    if DEBUG_Mario:
                        print("to                         : ",self.WPQ[(conv_P,0)].shape )
                        print("WPQ[(conv,1)]          : ",self.WPQ[(conv_P,1)].shape)
                else:
                    self.WPQ[(conv_P,0)] =  self.param_data(conv)
                    self.WPQ[(conv_P,1)] =  self.param_b_data(conv)
                    if DEBUG_Mario:
                        print(" @ First iter W2  @")
                        print("shape WPQ[(conv,0)] : ",self.WPQ[(conv_P,0)].shape )
                        print("shape WPQ[(conv,1)] : ",self.WPQ[(conv_P,1)].shape )
                ...
                if DEBUG_Mario:print(" conv layer name (X_name): ", conv)

                # newX: N c h w (BatchSize, channels, h, w), W2: n c h w (out_channels, in_channels, fitler_h, filter_w)
                idxs, W2, B2 = self.dictionary_kernel(X_name, None, d_c, convnext, None, DEBUG=True)
                if DEBUG_Mario:
                        print("--- dictionary_kernel ret --- ")
                ...
                if conv == alldic[-1]:
                    if DEBUG_Mario:
                        print(" %s, last run, add weights of %s to WPQ" %(dcfgs.model.model, convnext))
                    convnext_P = underline(convnext,'P')
                    self.WPQ[(convnext_P,0)] =  self.param_data(convnext)[:,self.selection[convnext],:,:]
                    self.WPQ[(convnext_P,1)] =  self.param_b_data(convnext)
                    self.set_conv(convnext, new_name=convnext_P)


                if DEBUG_Mario:
                    print("@ Set W2 @")
                    print("weights convnext that are set to zero: ", self.param_data(convnext)[:, ~idxs, ...].shape)
                    print("weights convnext that are retained   : ", self.param_data(convnext)[:, idxs, ...].shape)
                    print("weights covnext in model             : ", self.param_data(convnext).shape)


                # W1
                self.WPQ[(conv_P,0)] = self.WPQ[(conv_P,0)][idxs]
                self.WPQ[(conv_P,1)] = self.WPQ[(conv_P,1)][idxs]
                self.set_conv(conv, num_output=sum(idxs),new_name=conv_P)
                if DEBUG_Mario:
                    print("@ Prune filters W1 @")
                    print("WPQ[(conv,0)] ",self.WPQ[(conv_P,0)].shape)
                    print("WPQ[(conv,1)] ",self.WPQ[(conv_P,1)].shape)

                t.toc('channel pruning')
    ...
    def dictionary_kernel(self, X_name, weights, d_prime, Y_name, Y, DEBUG = 0):
        ...
        if not self._mem: # if we have not extracted sample features before, then extract them
            if DEBUG: print("have not extracted sample features before... calling self.extra_features")
        ...
        if DEBUG:
            print("X (ret of extract_XY):", X.shape )
            print("N (number or samples per channel):", N)
            print("Activation width & height:", w)
            print("newX: ", newX.shape)

        W2 = self.param_data(Y_name)
        if DEBUG: print("Shape of original  W2 weights: ", W2.shape)
        if dcfgs.ls != cfgs.solvers.gd or not self._mem: # add paramb # what is this? -by Mario
            if DEBUG:
                print("dcfgs.ls != gd --> %s , ~(self._mem) --> %s" % (dcfgs.ls != cfgs.solvers.gd , not self._mem) )
                print("then assigns gtY using the pre-loaded samples (net._feats_dict)")
...
def load_layer(convs, frozen_layer):
    ...
    def subfile(filename):
        return osp.join(frozen_layer, filename + '.pickle')

    filename = subfile(cfgs._points_dict_name)
    if osp.exists(filename):
        print("loading %s to %s" % ( filename , cfgs._points_dict_name))
        with open(filename, 'rb') as f:
    ...
    for conv in convs:
        filename = subfile(conv)
        if osp.exists(filename):
            print("loading %s to feats_dict[%s]"  %(filename,conv))
            with open(filename, 'rb') as f:
